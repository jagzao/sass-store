...
test.describe("Payments API Flow", () => {
  ...
  test.describe("Payment Authorization (Critical)", () => {
    test("should reject unauthenticated requests to create payment", async ({
      request,
    }) => {
    ...
    test("should reject unauthenticated requests to list all payments", async ({
      request,
    }) => {
    ...
    test("should reject unauthenticated requests to get payments by user", async ({
      request,
    }) => {
      ...
      expectStatusOneOf(
        response.status(),
        [401, 403],
        "unauthenticated list by user",
      );
    });

    test("should reject unauthenticated requests to update payment", async ({
      request,
    }) => {
      const response = await request.put(
        `/api/payments?paymentId=test_payment`,
        {
          headers: {
            "Content-Type": "application/json",
          },
    ...
    test("should reject unauthenticated requests to refund payment", async ({
      request,
    }) => {
  ...
  test.describe("Payment Validation (with auth rejection)", () => {
    test("should reject invalid payment method (auth checked first)", async ({
      request,
    }) => {
      const response = await request.post(`/api/payments`, {
        headers: {
    ...
    test("should reject negative amount (auth checked first)", async ({
      request,
    }) => {
      const response = await request.post(`/api/payments`, {
        headers: {
    ...
    test("should reject missing required fields (auth checked first)", async ({
      request,
    }) => {
      const response = await request.post(`/api/payments`, {
        headers: {
  ...
  test.describe("Payment API Structure", () => {
    ...
    test("payments refund endpoint should exist and respond", async ({
      request,
    }) => {
