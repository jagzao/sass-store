...
test.describe("Media Pipeline & Optimization", () => {
  ...
  test("uploaded images should be optimized and served with blurhash variants", async ({
    page,
    request,
  }) => {
    ...
    const productImages = page
      .locator('img[src*="/media/"], img[src*="cdn"]')
      .first();
    ...
    if (await productImages.isVisible()) {
      ...
      const hasPlaceholder =
        style?.includes("background") || style?.includes("data:image");
      if (hasPlaceholder) {
  ...
  test("API handles deduplication logic on media upload", async ({
    request,
  }) => {
    ...
    if (response.status() === 400 || response.status() === 422) {
      ...
      console.log(
        "Upload route might be under a different path in the new architecture.",
      );
    }
  });
});
