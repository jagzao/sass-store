...
test.describe("Quotas & Cost Guards", () => {
  ...
  test("Dashboard displays Quota Warnings at boundaries (80%, 90%)", async ({
    page,
  }) => {
    ...
    const ecoWarning = page.getByText(
      /You have reached 80%|Modo Eco activado/i,
    );
    const freezeWarning = page.getByText(
      /You have reached 90%|Modo Freeze|Solo lectura/i,
    );
    const killWarning = page.getByText(
      /You have reached 100%|Servicio bloqueado/i,
    );
  ...
  test("API returns 429 Too Many Requests when limits exceeded", async ({
    request,
  }) => {
    const { tenantSlug } = TEST_CREDENTIALS;
    ...
    const requests = Array(10)
      .fill(0)
      .map(() => request.get(`/api/v1/products?tenant=${tenantSlug}`));
