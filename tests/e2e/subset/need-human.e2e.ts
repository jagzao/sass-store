...
test.describe("NEED=HUMAN E2E Validation", () => {
  test("should trigger NEED=HUMAN when data-testid is missing", async ({
    page,
  }) => {
    ...
    log.info("QA", "need-human-test", "Starting NEED=HUMAN validation", {
      tenant,
    });
    ...
    if (!hasStableSelector) {
      ...
      const alertFile = alerts.missingTestId(
        "QA",
      ...
      if (foundWithFallback) {
        ...
        throw new Error(
          `NEED=HUMAN: Critical booking button not found. Alert created: ${alertFile}`,
        );
      }
    } else {
      ...
      const bookingConfirmation = page.locator(
        '[data-testid="booking-form"], [data-testid="booking-modal"]',
      );
  ...
  test("should validate color log output in console", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.testLogger = {
        info: (agent: string, task: string, msg: string) => {
          ...
          console.log(
            `[${timestamp}] INFO AGENT=${agent} TASK=${task} msg="${msg}"`,
          );
        },
        warn: (agent: string, task: string, msg: string) => {
          const timestamp = new Date().toTimeString().slice(0, 8);
          console.log(
            `[${timestamp}] ⚠ AGENT=${agent} TASK=${task} NEED=HUMAN msg="${msg}"`,
          );
        },
      };
    });
    ...
    await page.evaluate(() => {
      (window as any).testLogger.info(
        "UI",
        "page-load",
        "Page loaded successfully",
      );
      (window as any).testLogger.warn(
        "QA",
        "selector-test",
        "Testing NEED=HUMAN format",
      );
    });
    ...
    expect(
      consoleMessages.some(
        (msg) => msg.includes("NEED=HUMAN") && msg.includes("AGENT=QA"),
      ),
    ).toBe(true);
  ...
  test("should handle missing configuration gracefully", async ({ page }) => {
    ...
    const isAdminAccessible =
      (await page.locator('[data-testid="admin-dashboard"]').count()) > 0;
    const hasAuthError =
      (await page
        .locator('.auth-error, [data-testid="auth-required"]')
        .count()) > 0;
    ...
    if (hasAuthError || !isAdminAccessible) {
      ...
      log.warn(
        "AUTH",
        "config-check",
        "Admin access requires authentication setup",
        {
          case: "admin-access",
          need: "HUMAN",
          file: alertFile,
  ...
  test("should validate API error handling", async ({ page }) => {
    ...
    if (failedRequests.length > 0) {
      ...
      const alertFile = alerts.apiError(
        "API",
        failedEndpoint,
  ...
  test("should pass after data-testid is added", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      setTimeout(() => {
        ...
        buttons.forEach((button, index) => {
          if (
            button.textContent?.toLowerCase().includes("reserv") ||
    ...
    if ((await bookButton.count()) > 0) {
      ...
      const bookingModal = page.locator(
        '[data-testid="booking-modal"], [data-testid="booking-form"]',
      );
      ...
      log.warn(
        "QA",
        "e2e",
        "Still missing data-testid after resolution attempt",
      );
    }
  });
});
