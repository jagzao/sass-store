...
test.describe.serial("Product Visits Workflow", () => {
  ...
  test("should add products to a visit", async ({ page }) => {
    ...
    const addClientBtn = page
      .getByRole("button", {
        name: /Agregar Clienta|Crear Clienta|Nueva Clienta/i,
      })
      .first();
    ...
    await page.fill(
      'input[name="name"], input[placeholder*="ombre"]',
      customerName,
    );
    await page.fill(
      'input[type="tel"], input[placeholder*="eléfono"]',
      "5551234567",
    );
    ...
    await page.waitForURL(
      (url) =>
        url.pathname.includes("/clientes/") && !url.pathname.endsWith("/nueva"),
      { timeout: 10000 },
    );
    ...
    // Try text first
    const newVisitBtn = page.getByRole("button", {
      name: /Nueva Visita|Agregar Visita/i,
    });
