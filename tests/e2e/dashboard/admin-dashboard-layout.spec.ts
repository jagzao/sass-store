...
test.describe("Admin Dashboard Redesign - Full Width Layout", () => {
  ...
  test("should render the horizontal 3-row layout without the legacy sidebar", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("MONITOR DE RETOQUES", { exact: true }),
    ).toBeVisible();
  ...
  test("should render the bottom nav grid Fila Fila Fila cards correctly", async ({
    page,
  }) => {
