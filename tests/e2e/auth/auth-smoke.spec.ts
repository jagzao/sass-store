...
test.describe("Auth Regression Smoke Tests", () => {
  ...
  test.describe("Auth API Endpoints (Fast, No UI)", () => {
    test("should return session info from session endpoint", async ({
      request,
    }) => {
    ...
    test("should reject unauthenticated requests to protected payment API", async ({
      request,
    }) => {
      ...
      expectStatusOneOf(
        response.status(),
        [401, 403],
        "unauthenticated payments access",
      );
    });
    ...
    test("should handle signout request (even without session)", async ({
      request,
    }) => {
  ...
  test.describe("Login Page UI (Slower)", () => {
    ...
    test("should reject invalid credentials", async ({ page }) => {
      ...
      const hasError = await page
        .getByText(/inválidas|incorrect|invalid|error/i)
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Protected Resource Access", () => {
    test("should redirect to login when accessing admin page unauthenticated", async ({
      page,
    }) => {
