...
test.describe("Wondernails Carousel Hero Section", () => {
  ...
  test("should display the carousel container with correct structure", async ({
    page,
  }) => {
  ...
  test("should navigate to next slide when next button is clicked", async ({
    ...
    const initialActiveSlide = page.locator(
      '[data-testid="carousel-item"][data-index="1"]'
    );
    const initialTopic = await initialActiveSlide
      .locator(".topic")
      .textContent();
    ...
    // Wait for animation to complete by checking the data-is-animating attribute
    await expect(page.locator('[data-testid="carousel-container"]')).toHaveAttribute('data-is-animating', 'false');

    // The slide at index 1 should now have different content (array was reordered)
    const newActiveSlide = page.locator(
      '[data-testid="carousel-item"][data-index="1"]'
    );
  ...
  test("should navigate to previous slide when prev button is clicked", async ({
    page,
  }) => {
    ...
    const newTopic = await newActiveSlide
      .locator(".introduce .topic")
      .textContent();
  ...
  test("should show detail view when see more button is clicked", async ({
    page,
  }) => {
    ...
    const seeMoreButton = activeSlide.locator(
    ...
    await expect(
      activeSlide.locator('[data-testid="detail-specifications"]')
    ).toBeVisible();
    await expect(
      activeSlide.locator('[data-testid="back-button"]')
    ).toBeVisible();
  });

  test("should return to carousel view when back button is clicked", async ({
    page,
  }) => {
    ...
    await expect(
      activeSlide.locator('[data-testid="detail-view"]')
    ).toBeVisible();
  ...
  test("should display slide content correctly", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await expect(page.locator('[data-testid="carousel-container"]')).toHaveAttribute('data-is-animating', 'false'); // Wait for animation

      // Verify active slide is still visible and at index 1
      const currentActiveSlide = page.locator(
        '[data-testid="carousel-item"][data-index="1"]'
      );
      await expect(currentActiveSlide).toBeVisible();
      await expect(
        currentActiveSlide.locator(".introduce .topic")
      ).toBeVisible();
    }
  });
  ...
  test("should have proper keyboard navigation support", async ({ page }) => {
    ...
    const backActiveSlide = page.locator(
      '[data-testid="carousel-item"][data-index="1"]'
    );
  ...
  test("should have proper ARIA attributes for accessibility", async ({
    ...
    await expect(page.locator('[data-testid="next-button"]')).toHaveAttribute(
      "aria-label"
    );
    await expect(page.locator('[data-testid="prev-button"]')).toHaveAttribute(
      "aria-label"
    );
  });
  ...
  test("should have isolated styles that do not affect other page elements", async ({
    page,
  }) => {
    ...
    const headerStylesAfter = await header.evaluate((el) =>
      getComputedStyle(el)
    );
  ...
  test("should handle rapid clicking without breaking", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await expect(page.locator('[data-testid="carousel-container"]')).toHaveAttribute('data-is-animating', 'false'); // Wait for controls to re-enable
    }
  ...
  test("should maintain performance with smooth animations", async ({
    page,
  }) => {
