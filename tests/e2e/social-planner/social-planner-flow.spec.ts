...
test.describe.skip("Social Planner Flow", () => {
  ...
  test("should load planner, create post, schedule and verify variations", async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/Planner|Calendario|Mes/i).first()).toBeVisible(
      { timeout: 10000 },
    );

    // 2. Crear post (assuming there is a "Nuevo Post" or "+" button)
    const newPostButton = page
      .getByRole("button", { name: /Nuevo Post|Crear/i })
      .first();
    ...
    if (btnVisible) {
      ...
      await expect(
        page
          .getByRole("dialog", { name: /Crear|Publicación/i })
      ...
      await page
        .fill(
          'input[name="title"], input[placeholder*="título" i]',
          "Test Post Title - " + Date.now(),
        )
        .catch(() => {});
      await page
        .fill(
          'textarea[name="content"], textarea[placeholder*="Escribe algo" i]',
          "Esta es mi nueva publicacion en el planner! #testing",
        )
        .catch(() => {});
      ...
      const facebookCheckbox = page.locator(
        'input[type="checkbox"][value="facebook"], input[name*="facebook" i]',
      );
      if (await facebookCheckbox.isVisible()) await facebookCheckbox.check();

      const instagramCheckbox = page.locator(
        'input[type="checkbox"][value="instagram"], input[name*="instagram" i]',
      );
      ...
      const uploadButton = page.locator(
        'button:has-text("Subir"), input[type="file"]',
      );
      if (
        (await uploadButton.isVisible()) &&
        (await uploadButton.getAttribute("type")) === "file"
      ) {
      ...
      const scheduleButton = page
        .getByRole("button", { name: /Programar|Guardar/i })
        .first();
      ...
      const newPostEl = page
        .locator('.scheduled-post, [data-testid="social-post"]')
        .first();
      if (await newPostEl.isVisible()) {
        ...
        if (postBounds) {
          ...
          await page.mouse.move(postBounds.x + 10, postBounds.y + 100, {
            steps: 5,
          });
          await page.mouse.up();
        }
      ...
      if (await yearViewButton.isVisible()) {
        ...
        await expect(
          page.locator('.heatmap, [data-testid="heatmap"]'),
        ).toBeVisible();
      }
    }
  });
