...
test.describe("Multitenant Isolation", () => {
  test("cart and session state should be isolated between tenants", async ({
    page,
  }) => {
    ...
    const productButton = page
      .locator('button:has-text("Agregar"), button:has-text("Reservar")')
      .first();
    ...
    if (await productButton.isVisible()) {
      ...
      const cartCounter = page.locator(
    ...
    const lsTenant1 = await page.evaluate(() =>
    ...
    const cartCounterTenant2 = page.locator(
      '[data-testid="cart-count"], .cart-count',
    );
  ...
  test("API endpoints should enforce tenant isolation", async ({
    page,
    request,
  }) => {
