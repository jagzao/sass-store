...
test.describe("Wondernails - Performance & Bundle Isolation", () => {
  test("bundle de HeroWondernails NO debe descargarse en otros tenants", async ({
    page,
  }) => {
    ...
    const wondernailsBundles = networkRequests.filter(
      (url) =>
  ...
  test("bundle de HeroWondernails SÍ debe descargarse solo en wondernails", async ({
    page,
  }) => {
    const networkRequests: string[] = [];
    ...
    page.on("response", async (response) => {
      ...
      if (
        (url.includes(".js") || url.includes("chunk")) &&
        response.status() === 200
      ) {
        networkRequests.push(url);
    ...
    const wondernailsChunks = chunkContents.filter(
      (content) =>
        content.includes("HeroWondernails") ||
        content.includes("wondernails") ||
    ...
    console.log(
      `Chunks con contenido de Wondernails: ${wondernailsChunks.length}`,
    );
  });

  test("debe medir el tamaño del bundle de HeroWondernails", async ({
    page,
  }) => {
    ...
    console.log(
      `✅ Tamaño total del bundle: ${(totalSize / 1024).toFixed(1)}KB`,
    );
    chunkSizes.forEach((chunk) => {
      console.log(
        `  - ${chunk.url.split("/").pop()}: ${(chunk.size / 1024).toFixed(1)}KB`,
      );
    });
  });

  test("debe medir LCP (Largest Contentful Paint) del hero", async ({
    page,
  }) => {
    await page.goto("/t/wondernails");
  ...
  test("debe medir FPS durante animaciones de transición", async ({ page }) => {
    ...
    console.log(
      `✅ FPS durante animación: ${Math.round(fps as number)}fps (objetivo: >50fps)`,
    );
  });

  test("debe verificar que no hay memory leaks en auto-rotación", async ({
    page,
  }) => {
    await page.goto("/t/wondernails");
    await page.waitForSelector('[data-tenant-hero="wondernails"]');
    ...
    console.log(
      `✅ Incremento de memoria: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB (objetivo: <5MB)`,
    );
  });

  test("debe verificar cleanup de timers al cambiar de página", async ({
    page,
  }) => {
    await page.goto("/t/wondernails");
    await page.waitForSelector('[data-tenant-hero="wondernails"]');
    ...
    const timersBeforeLeave = await page.evaluate(() => {
      ...
      return Object.keys(window).filter(
        (key) => key.includes("timer") || key.includes("interval"),
      ).length;
    });
    ...
    console.log(
      `✅ Timers cleanup: ${timersBeforeLeave} -> ${timersAfterLeave}`,
    );
  });
  ...
  test("debe medir tiempo de hidratación del componente", async ({ page }) => {
    ...
    console.log(
      `✅ Tiempo de hidratación: ${hydrationTime}ms (objetivo: <1000ms)`,
    );
  });

  test("debe verificar que CSS Modules no contaminan el scope global", async ({
    page,
  }) => {
    await page.goto("/t/wondernails");
    await page.waitForSelector('[data-tenant-hero="wondernails"]');
    ...
    console.log(
      `✅ CSS Modules: ${hashedClasses.length} clases hasheadas, 0 conflictos globales`,
    );
  });
});
