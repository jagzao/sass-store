...
describe("Workflow Reform Validation", () => {
  ...
  describe("3. Auto-continuación (resume)", () => {
    ...
    it("should have autoresume configuration", () => {
      ...
      expect(config.maxRetries).toBe(3); // Updated to match actual config
    });
  });
  ...
  describe("5. Auto-reparación (autofix)", () => {
    ...
    it("should have patcher subagents", () => {
      const patcherFrontend = join(
        projectRoot,
        "agents",
        "sub",
        "patcher-frontend.md",
      );
      const patcherBackend = join(
        projectRoot,
        "agents",
        "sub",
        "patcher-backend.md",
      );
      const patcherInfra = join(
        projectRoot,
        "agents",
        "sub",
        "patcher-infra.md",
  ...
  describe("MCP Configurations", () => {
    ...
    it("should have MCP web-search configuration", () => {
      ...
      expect(config.search_strategies.technical_documentation.enabled).toBe(
        true,
      );
