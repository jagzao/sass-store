...
test.describe("API Integration Tests", () => {
  ...
  test("Media upload API should optimize images", async ({ request }) => {
    ...
    if (response.status() === 200) {
      ...
      if (result.variants) {
        ...
        expect(
          variantUrls.includes(".webp") || variantUrls.includes(".avif"),
        ).toBeTruthy();
      }

      // Should include metadata
      expect(
        result.metadata || result.blurhash || result.dominantColor,
      ).toBeTruthy();
    }
  });
  ...
  test("Social Planner API should handle post scheduling", async ({
    request,
  }) => {
    const tenant = "wondernails";
    ...
    if (response.status() === 200 || response.status() === 201) {
      ...
      const getResponse = await request.get(
        `${baseURL}/api/v1/social/posts/${post.id}`,
        {
          headers: {
            "x-tenant": tenant,
          },
