-- =====================================================
-- UPSERT CATALOG - SAFE FOR PRODUCTION
-- Last Updated: 2024-12-12
-- This script safely updates catalogs without deleting production data
-- =====================================================
--
-- WHAT THIS SCRIPT DOES:
-- ✅ Inserts new tenants if they don't exist, updates if they do
-- ✅ Inserts/updates products and services (catalogs)
-- ✅ Inserts/updates staff (team members)
-- ❌ DOES NOT touch customers, visits, or user data
-- ❌ DOES NOT delete anything
--
-- SAFE TO RUN: Multiple times, in production, during development
-- =====================================================

BEGIN;

-- =====================================================
-- TENANTS - UPSERT (Insert or Update)
-- =====================================================
...
('wondernails', 'Wonder Nails Studio', 'Salón de uñas premium especializado en manicure, pedicure y nail art de alta calidad', 'booking', 'active',
  ...
  '{"email": "marialiciavh1984@gmail.mx", "phone": "+52 1 55 6406 8409", "website": "https://wondernails.mx", "address": "Manzana 012, San Lorenzo, 56140 Texcoco, State of Mexico, Mexico", "googleMaps": "https://maps.app.goo.gl/FS471vtXdFdPTyjEA", "hours": {"monday": "10:00-20:00", "tuesday": "10:00-20:00", "wednesday": "10:00-20:00", "thursday": "10:00-20:00", "friday": "10:00-21:00", "saturday": "9:00-21:00", "sunday": "10:00-18:00"}}',
...
-- ON CONFLICT: If slug exists, update all fields
...
-- =====================================================
-- PRODUCTS - UPSERT (Insert or Update)
-- =====================================================
-- Uses tenant_id + sku as unique identifier

-- First, delete old products that are no longer in the catalog
-- This helps keep the catalog clean during development
WITH catalog_skus AS (
  SELECT unnest(ARRAY[
    'ZS-SAAS-PRO', 'ZS-ECOM-TEMP',
    'WN-POLISH-RUBY', 'WN-POLISH-PINK', 'WN-CUTICLE-OIL', 'WN-HAND-CREAM', 'WN-NAIL-FILE', 'WN-REMOVER',
    'VS-SHAMPOO-REPAIR', 'VS-COND-HYDRA', 'VS-SERUM-SHINE', 'VS-SPRAY-HEAT',
    'CT-RACKET-PRO', 'CT-BALLS-PENN', 'CT-SHOES-NIKE', 'CT-BAG-WILSON',
    'DEL-BUDDHA-BOWL', 'DEL-POKE-SALMON', 'DEL-SMOOTHIE-GREEN', 'DEL-ENSALADA-GRIEGA', 'DEL-WRAP-POLLO', 'DEL-JUICE-NARANJA',
    'NN-TACOS-PASTOR', 'NN-TACOS-CARNITAS', 'NN-TACOS-BISTEC', 'NN-QUESADILLA-QUESO', 'NN-TORTA-MILANESA'
  ]) AS sku
)
DELETE FROM products
WHERE sku NOT IN (SELECT sku FROM catalog_skus);
...
-- ON CONFLICT: If tenant_id + sku exists, update the product
...
-- =====================================================
-- SERVICES - UPSERT (Insert or Update)
-- =====================================================
-- Delete old services that are no longer in catalog
WITH catalog_services AS (
  SELECT unnest(ARRAY[
    'Manicure Gel Completo', 'Pedicure Spa Deluxe', 'Nail Art Diseño Básico', 'Uñas Acrílicas Esculpidas',
    'Corte y Styling Premium', 'Color Completo Profesional', 'Tratamiento Kerastase Fusio-Dose', 'Peinado para Evento',
    'Clase Individual de Tenis', 'Clase Grupal Tenis (4 personas)', 'Renta de Cancha por Hora', 'Clínica de Tenis Fin de Semana',
    'Plan Semanal Saludable', 'Catering Saludable Eventos',
    'Taquiza para 10 Personas', 'Delivery Express'
  ]) AS service_name
)
DELETE FROM services
WHERE name NOT IN (SELECT service_name FROM catalog_services);
...
-- ON CONFLICT: If tenant_id + name exists, update
...
-- ON CONFLICT: If tenant_id + email exists, update
...
-- - 6 Tenants (upserted)
-- - 27 Products (upserted, old ones deleted)
-- - 16 Services (upserted, old ones deleted)
-- - 7 Staff members (upserted)
--
-- ❌ NOT touched:
-- - Customers (production data)
-- - Customer visits (production data)
