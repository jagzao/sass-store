import type { ClassifiedIntent, IntentType, IntentEntities } from "../types";

/** Mapa de keywords a intent con peso de confianza */
const INTENT_PATTERNS: Array<{
  ...
  },
  {
    ...
    patterns: [
      /\b(tienen?|hay|tienen?)\s+(hora|cita|lugar|espacio|disponibilidad)\b/i,
      /\bdisponibilidad\b/i,
  ...
  },
  {
    intent: "hours_query",
    patterns: [
      /\b(horario|a\s+qu[eé]\s+hora|hasta\s+cu[aá]ndo|abren?|cierran?)\b/i,
    ],
    baseConfidence: 0.9,
  },
  {
  ...
  },
  {
    intent: "confirm_yes",
    patterns: [
      /^(s[ií]|dale|va|confirmar?|perfecto|listo|ok|okay|si quiero|de acuerdo)\s*[.!]?$/i,
    ],
    baseConfidence: 0.95,
  },
  {
    intent: "confirm_no",
    patterns: [/^(no|cancelar?|mejor no|nope|nel|para|ya no)\s*[.!]?$/i],
    baseConfidence: 0.95,
  },
  {
    intent: "human_request",
    patterns: [
      /\b(hablar?\s+con\s+(alguien|una?\s+persona|humano)|persona\s+real|no\s+(quiero|quier)\s+(bot|robot))\b/i,
    ],
    baseConfidence: 0.98,
  },
];
...
export class IntentClassifier {
  classify(text: string, buttonPayload?: string): ClassifiedIntent {
    // Botones interactivos son siempre alta confianza
  ...
  private extractEntities(text: string): IntentEntities {
    ...
    // Servicio (keywords básicos — se complementa con KB del tenant en el handler)
    const servicePatterns: Array<{ pattern: RegExp; label: string }> = [
