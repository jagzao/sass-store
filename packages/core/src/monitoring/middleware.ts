...
export class MonitoringMiddleware {
    ...
    try {
      ...
      const errorId = await captureError(
        error as Error,
        {
          ...
          user: context.userId
            ? { id: context.userId, tenantId: context.tenantId || "" }
            : undefined,
          metadata: {
  ...
  private generateRequestId(): string {
    return `req_${Date.now()}_${crypto.randomUUID().replace(/-/g, "").substring(0, 9)}`;
  }
  ...
  private getPathPattern(pathname: string): string {
    ...
    return pathname
      ...
      .replace(
        /\/wondernails|\/vigistudio|\/zo-system|\/nom-nom|\/delirios|\/centro-tenistico|\/vainilla-vargas/g,
        "/[tenant]",
  ...
  private getActiveRequestCount(): number {
    ...
    return Array.from(this.activeRequests.values()).reduce(
      (sum, count) => sum + count,
      0,
    );
  }
