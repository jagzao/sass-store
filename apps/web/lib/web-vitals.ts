...
export function reportWebVitals(metric: any) {
  ...
  if (process.env.NODE_ENV === "production") {
    // AquÃ­ se podrÃ­a integrar con servicios como:
    // - Vercel Analytics
    // - Google Analytics 4
    // - Sentry
    // - DataDog
    // Por ahora solo log
    console.warn(`[WebVitals] ${name}: ${value}`);
  }
}
...
export function initWebVitals() {
  ...
  // Solo inicializar si es producciÃ³n o si se fuerza en desarrollo
  if (
    process.env.NODE_ENV === "production" ||
    process.env.NEXT_PUBLIC_TRACK_VITALS === "true"
  ) {
