...
export class CartService {
  ...
  async addToCart(
    ...
    if (existingItemIndex >= 0) {
      ...
      // Add new item - the mock DB's insertCartItem will add it to the cart
      const itemData = {
        cartId: cart.id,
      ...
      // Don't push here - insertCartItem already handles it
    }
  ...
  async updateCartItem(
    ...
    const item = await this.db.findCartItem(cart.id, request.itemId);
    if (!item) {
      return Err(ErrorFactories.notFound("Cart", request.cartId));
    }
  ...
  async removeFromCart(
    ...
    const status = updatedItems.length === 0 ? "abandoned" : cart.status;
    ...
    const updatedCart = {
      ...
      subtotal: newSubtotal,
      total: newSubtotal,
      status: status as "active" | "abandoned" | "checkout",
      updatedAt: new Date(),
    };
  ...
  async validateCartForCheckout(
    ...
    if (cart.items.length === 0) {
      return Err(
        ErrorFactories.businessRule("items", "Cart cannot be empty for checkout"),
      );
    }
...
// Export singleton instance (for convenience, but prefer dependency injection)
export const cartService = new CartService({} as CartDatabase);
