...
export const orders = pgTable(
  ...
  (table) => ({
    ...
    paymentIntentIdx: index("order_payment_intent_idx").on(
...
export const mediaAssets = pgTable(
  "media_assets",
  {
    ...
    entityId: uuid("entity_id"), // ID of related entity (product, service, etc.)
...
export const customers = pgTable(
  ...
  (table) => ({
    ...
    retouchDateIdx: index("customer_retouch_date_idx").on(
      table.nextRetouchDate,
    ),
  }),
);
...
export const customerVisits = pgTable(
  ...
  (table) => ({
    tenantIdx: index("customer_visit_tenant_idx").on(table.tenantId),
    customerIdx: index("customer_visit_customer_idx").on(table.customerId),
    serviceIdx: index("customer_visit_service_idx").on(table.serviceId),
    visitDateIdx: index("customer_visit_date_idx").on(table.visitDate),
    statusIdx: index("customer_visit_status_idx").on(table.status),
  }),
);

// Service Retouch Config table
export const serviceRetouchConfig = pgTable(
  "service_retouch_config",
  {
    ...
    serviceId: uuid("service_id")
      .references(() => services.id)
      .notNull(),
    frequencyType: varchar("frequency_type", { length: 20 }).notNull(), // 'days' | 'weeks' | 'months'
  ...
  (table) => ({
    tenantIdx: index("retouch_config_tenant_idx").on(table.tenantId),
    serviceIdx: index("retouch_config_service_idx").on(table.serviceId),
    activeIdx: index("retouch_config_active_idx").on(table.isActive),
    tenantServiceUniqueIdx: uniqueIndex(
      "retouch_config_tenant_service_unique_idx",
    ).on(table.tenantId, table.serviceId),
  }),
);

// Tenant Holidays table
export const tenantHolidays = pgTable(
  "tenant_holidays",
  {
    ...
    affectsRetouch: boolean("affects_retouch").notNull().default(true),
    description: text("description"),
    metadata: jsonb("metadata"),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  ...
  (table) => ({
    tenantIdx: index("holiday_tenant_idx").on(table.tenantId),
    dateIdx: index("holiday_date_idx").on(table.date),
    affectsRetouchIdx: index("holiday_affects_retouch_idx").on(
      table.affectsRetouch,
    ),
    tenantDateUniqueIdx: uniqueIndex("holiday_tenant_date_unique_idx").on(
      table.tenantId,
      table.date,
    ),
  }),
...
export const customerVisitsRelations = relations(customerVisits, ({ one }) => ({
  ...
  service: one(services, {
    fields: [customerVisits.serviceId],
    references: [services.id],
  }),
}));

// Service Retouch Config Relations
