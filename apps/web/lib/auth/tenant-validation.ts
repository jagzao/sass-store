...
export async function validateTenantAccess(request: NextRequest) {
  ...
  // If the user's session tenant doesn't match the current tenant, validate access
  if (session.user.tenantSlug !== tenantSlug) {
    try {
      ...
      try {
        ...
        console.error(
          "[TenantValidation] Failed to set tenant context:",
          error,
        );
        return NextResponse.redirect(
      ...
      // Update the session with the current tenant context
      // This is important for maintaining consistency across the application
      const response = NextResponse.next();

      // Add headers to indicate successful tenant validation
      ...
      console.error(
        "[TenantValidation] Error validating tenant access:",
        error,
      );
      return NextResponse.redirect(
  ...
  // If tenant matches session tenant, allow the request to continue
  return NextResponse.next();
}
