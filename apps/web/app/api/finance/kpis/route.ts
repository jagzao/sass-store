...
const validateKPIQuery = (
  searchParams: URLSearchParams,
): Result<{ period: string; tenant: string }, DomainError> => {
...
const getKPIData = async (
  period: string,
  tenantSlug: string,
): Promise<Result<KPIData, DomainError>> => {
  try {
    // 1. Get Tenant ID
    ...
    if (!tenantResult.length) {
      return Err(
        ErrorFactories.notFound("TENANT_NOT_FOUND", "Tenant not found"),
      );
    }
    ...
    const totalIncome =
      parseFloat(ordersResult[0].total) + parseFloat(visitsResult[0].total);
    const transactionCount =
      Number(ordersResult[0].count) + Number(visitsResult[0].count);
    ...
    const approvalRate =
      Number(paymentsResult[0].total) > 0
        ? (Number(paymentsResult[0].completed) /
