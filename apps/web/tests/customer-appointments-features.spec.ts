...
test.describe("Customer Management & Appointments Dashboard", () => {
  ...
  test("should load the admin dashboard and navigate to customers", async ({
    page,
  }) => {
  ...
  test("should explicitly display the 'Precio a Cobrar' in the dashboard appointments section", async ({
    page,
  }) => {
    await page.goto(`/t/${TENANT_SLUG}/admin`);
    ...
    const sectionTitle = page.getByText(
      /NUEVAS CLIENTAS POR CONFIRMAR ESTA SEMANA/i,
    );
    ...
    const noAppointmentsMsg = page.getByText(
      /Todas las citas están confirmadas/i,
    );
  ...
  test("should display live real prices and columns in the Calendar page", async ({
    page,
  }) => {
    ...
    const noAppointmentsTodayMsg = page.getByText(
      /No hay citas registradas para hoy/i,
    );
  ...
  test("should navigate to customers and have the new birthday and medical history fields in the form", async ({
    page,
  }) => {
    ...
    const createButton = page
      .getByRole("button", { name: /\+ Nueva Clienta|Crear/i })
      .first();
    if (await createButton.isVisible()) {
      ...
      await expect(
        page.getByText(/Condiciones Médicas \(Diabetes, Psoriasis/i),
      ).toBeVisible();
