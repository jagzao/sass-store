    ...
    async function fetchPendingCustomers() {
      try {
        ...
        const response = await fetch(
          `/api/tenants/${tenantSlug}/customers?sort=lastVisit&order=desc`,
        );
        ...
        const customersToConfirm = (data.customers || [])
          .filter(
            (customer: any) => customer.lastVisit && !customer.nextAppointment,
          )
          .map((customer: any) => {
          ...
          .filter(
            (mapped: any) =>
              mapped.daysSinceLastVisit >= 14 &&
              mapped.daysSinceLastVisit <= 20,
          )
          .sort(
            (a: any, b: any) => b.daysSinceLastVisit - a.daysSinceLastVisit,
          ); // Sort oldest first (closest to 20 days)
