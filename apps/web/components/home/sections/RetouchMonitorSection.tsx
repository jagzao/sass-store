  ...
  useEffect(() => {
    async function fetchRetouches() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            "Error al obtener los clientes sugeridos para retoque",
          );
        }
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          const sorted = data.data.sort(
            (a: any, b: any) => b.daysSinceLastVisit - a.daysSinceLastVisit,
          );
          setRetouches(sorted);
        } else {
          throw new Error(
            data.error?.message || "Estructura de datos inválida",
          );
        }
      } catch (err) {
        ...
        {loading ? (
          ...
          <div className="bg-[#E6E6FA]/20 border border-[#C5A059]/10 rounded-xl p-8 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">
              No hay clientas en el umbral de retoque por el momento.
            </p>
          </div>
        ) : (
          retouches.map((retouch) => {
            const tl = getTrafficLight(retouch.daysSinceLastVisit);
            const dateStr = new Date(retouch.lastVisitDate).toLocaleDateString(
              "es-MX",
            ...
            const phoneClean = retouch.phone
              ? retouch.phone.replace(/\D/g, "")
              : "";
            const whatsappLink = phoneClean
              ? `https://wa.me/${phoneClean}?text=${encodeURIComponent(reminderMessage)}`
              : "#";
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`${tl.badgeBg} ${tl.badgeText} px-2 py-0.5 text-xs font-bold rounded-full`}
                      >
                        {tl.emoji} {tl.label}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      <span className="ml-2 text-xs opacity-75">
                        ({retouch.daysSinceLastVisit} días transcurridos)
                      </span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:flex-shrink-0">
                    {phoneClean ? (
                      ...
                      <span className="text-xs text-gray-400 px-3 py-2 bg-white rounded-lg border">
                        Sin teléfono
                      </span>
                    )}
                  </div>
