...
export function Trending() {
  ...
  const { buyNow, bookService, isLoading } = useCart();
  ...
  const getTrendingBadge = (trending: string) => {
    switch (trending) {
      ...
      case 'deal':
        return { text: '💰 OFERTA', bg: 'bg-green-100', color: 'text-green-800' };
      default:
        return { text: '', bg: '', color: '' };
    }
  };

  const handleAction = async (item: TrendingItem) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute top-3 left-3 z-10">
                <div className={`${badge.bg} ${badge.color} px-2 py-1 rounded-full text-xs font-bold shadow-sm`}>
                  {badge.text}
                </div>
              </div>
                  ...
                  {item.originalPrice ? (
                    <div className="space-y-1">
                      <div className="flex items-center justify-center space-x-2">
                        <span className="text-lg font-bold" style={{ color: 'var(--color-brand, #DC2626)' }}>${item.price}</span>
                        <span className="text-sm text-gray-600 line-through">${item.originalPrice}</span>
                      </div>
                    </div>
                  ) : (
                    <span className="text-lg font-bold" style={{ color: 'var(--color-brand, #DC2626)' }}>${item.price}</span>
