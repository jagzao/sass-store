...
const items: CarouselItem[] = [
  {
    ...
    description:
      "Deliciosa torta de chocolate con crema batida y fresas frescas",
  },
  {
    id: 2,
    image: "🧁",
    ...
    description:
      "Cupcakes artesanales con decoraciones únicas y sabores exóticos",
  },
  {
    id: 3,
    image: "🍪",
  ...
  },
  {
    ...
    description:
      "Flanes, mousses y cremas preparadas con recetas tradicionales",
  },
];
...
export function DeliriosCarousel({ tenantData }: DeliriosCarouselProps) {
  ...
  useEffect(() => {
    ...
    const textCircle =
      "DELIRIOS BAKERY - SABORES ÚNICOS - POSTRES ARTESANALES - ";
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-gray-900 font-sans">
      ...
      <header className="h-12 flex justify-between items-center px-12 relative z-10">
        <div className="text-white font-bold">
          {tenantData.name.toUpperCase()}
        </div>
        <nav>
          <ul className="flex gap-8 p-0 m-0 list-none text-white">
            <li>
              <a href={`/t/${tenantData.slug}`} className="hover:opacity-80">
                INICIO
              </a>
            </li>
            <li>
      ...
      <div className="slider relative w-screen h-screen overflow-hidden -mt-12">
          ...
          {items.map((item, index) => (
            <div
              ...
              style={{
                width: `${radius * 2}px`,
                transform:
                  index === carousel.active ? "rotate(0deg)" : "rotate(45deg)",
              }}
            >
        ...
        <div className="content absolute bottom-[5%] left-1/2 transform -translate-x-1/2 text-center text-white w-max">
          <div className="text-left uppercase transform translate-y-5">
            menú
          </div>
          <div className="text-6xl uppercase tracking-[10px] font-bold relative">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  "url(data:image/svg+xml;base64,)",
              }}
            />
          </div>
          ...
          <button className="border border-white/50 bg-transparent text-white font-sans tracking-[5px] rounded-2xl px-5 py-3 hover:bg-white/10 transition-colors">
            Ver Más
          </button>
        </div>
