...
const FormTextarea = memo(
  ({
    ...
    const textareaId =
      id ||
      props.name ||
      `textarea-${crypto.randomUUID().replace(/-/g, "").substring(0, 9)}`;
    ...
    const finalTextareaClassName =
      `${baseTextareaClassName} ${stateClassName} ${disabledClassName} ${textareaClassName}`.trim();

    const currentLength = typeof value === "string" ? value.length : 0;
    const showCounter =
      showCharCount && (maxLength !== undefined || currentLength > 0);
      ...
      <div className={containerClassName}>
        ...
        {showCounter && (
          <div className="flex justify-between items-center mt-1">
            ...
            <p
              className={`text-xs ml-2 ${currentLength === maxLength ? "text-red-600 font-medium" : "text-gray-500"}`}
            >
              {currentLength}
