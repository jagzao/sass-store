...
export function ScheduleTimeline() {
  ...
  const sortOptions = useMemo<SelectOption[]>(
    () => [
      { value: "publish_time_asc", label: "Fecha ↑" },
      { value: "publish_time_desc", label: "Fecha ↓" },
        ...
        {isLoading ? (
          ...
          <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
            ...
            <p className="text-gray-600">
              No se encontraron posts programados con los filtros aplicados.
            </p>
          </div>
        ) : (
            ...
            <div key={dateKey} className="space-y-4">
              ...
              <div className="sticky top-0 z-10 bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <p className="text-gray-600 text-sm">
                  {groupedPosts[dateKey].length} publicacion
                  {groupedPosts[dateKey].length !== 1 ? "es" : ""} programada
                  {groupedPosts[dateKey].length !== 1 ? "s" : ""}
                </p>
              </div>
              ...
              <div className="space-y-3 ml-6">
                {groupedPosts[dateKey]
                  .sort((a, b) => {
                    const timeA = a.targets[0].publishAtUtc
                      ? new Date(a.targets[0].publishAtUtc).getTime()
                      : 0;
                    const timeB = b.targets[0].publishAtUtc
                      ? new Date(b.targets[0].publishAtUtc).getTime()
                      : 0;
                    return timeA - timeB;
                  })
                  .map((post, index) => {
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-shrink-0 text-center">
                            <div className="text-sm font-medium text-gray-900">
                              {target.publishAtUtc
                                ? format(new Date(target.publishAtUtc), "HH:mm")
                                : "--:--"}
                            </div>
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-gray-700 text-sm mb-2">
                              {truncateText(
                                target.variantText || post.baseText,
                              )}
                            </p>
                            ...
                            {target.assetIds && target.assetIds.length > 0 && (
                              <div className="flex items-center space-x-1 text-xs text-gray-500">
                                ...
                                <span>
                                  {target.assetIds.length} imagen
                                  {target.assetIds.length !== 1 ? "es" : ""}
                                </span>
                              </div>
                            )}
                          </div>
                            ...
                            >
                              ...
                              >
                                ...
                                <path
                                  ...
                                  strokeLinejoin="round"
                                  strokeWidth={2}
                                  d="M2.458 12C3.732 7.943 7.523 5 12c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7z"
                                />
                              </svg>
                            </button>
                          </div>
