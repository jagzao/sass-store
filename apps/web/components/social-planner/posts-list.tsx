    ...
    try {
      ...
      await Promise.all(
        (result.data || []).map(async (post: SocialPost) => {
          try {
            const targetsResponse = await fetch(`/api/v1/social/posts/${post.id}/targets`);
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
            ...
            <p className="text-gray-600">No se encontraron posts con los filtros aplicados.</p>
          </div>
        ) : (
          posts.map(post => (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_CONFIG[post.status].color}`}>
                      {STATUS_CONFIG[post.status].label}
                    </span>

                    {post.scheduledAtUtc && (
                      <span className="text-sm text-gray-500">
                        📅 {format(new Date(post.scheduledAtUtc), 'dd/MM/yyyy HH:mm', { locale: es })}
                      </span>
                    )}
                  </div>
                      ...
                      <div className="flex items-center space-x-2">
                        {targets[post.id].map((target, index) => (
                          <div key={index} className="flex items-center space-x-1 bg-gray-100 rounded-full px-2 py-1">
                            <span>{PLATFORM_CONFIG[target.platform as keyof typeof PLATFORM_CONFIG]?.emoji || '📱'}</span>
                            <span className="text-xs font-medium">
                              {PLATFORM_CONFIG[target.platform as keyof typeof PLATFORM_CONFIG]?.name || target.platform}
                            </span>
                            <span className={`text-xs px-1 py-0.5 rounded ${STATUS_CONFIG[target.status as keyof typeof STATUS_CONFIG]?.color || 'bg-gray-100 text-gray-800'}`}>
                              {STATUS_CONFIG[target.status as keyof typeof STATUS_CONFIG]?.label || target.status}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="text-sm text-gray-500">
                    Creado {format(new Date(post.createdAt), 'dd/MM/yyyy HH:mm', { locale: es })}
