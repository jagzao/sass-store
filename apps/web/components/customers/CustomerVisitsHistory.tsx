...
export default function CustomerVisitsHistory({
  ...
  return (
    <>
      <div
        className={`${isLuxury ? "bg-white/80 border border-[#D4AF37]/20 backdrop-blur-md shadow-sm" : "bg-white shadow"} rounded-lg overflow-hidden`}
      >
        <div
          className={`p-6 border-b ${isLuxury ? "border-[#D4AF37]/10" : "border-gray-200"} flex justify-between items-center`}
        >
          <h2
            className={`text-xl font-semibold ${isLuxury ? "text-[#1a1a1a] font-serif" : "text-gray-900"}`}
          >
            Historial de Visitas ({visits.length})
          </h2>
          <button
            onClick={handleAddVisit}
            data-testid="btn-new-visit"
        ...
        {visits.length === 0 ? (
          <div className="p-12 text-center">
            <Calendar
              className={`mx-auto h-12 w-12 ${isLuxury ? "text-[#D4AF37]/30" : "text-gray-400"} mb-4`}
            />
            <h3
              className={`text-lg font-medium ${isLuxury ? "text-gray-900" : "text-gray-900"} mb-2`}
            >
              Sin visitas registradas
            </h3>
            <p
              className={`${isLuxury ? "text-gray-500" : "text-gray-600"} mb-6`}
            >
              Registra la primera visita de esta clienta para comenzar su
              historial.
            </p>
            <button
              onClick={handleAddVisit}
              data-testid="btn-new-visit-empty"
              className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium ${isLuxury ? "text-white bg-[#D4AF37] hover:bg-[#b3932d]" : "text-white bg-blue-600 hover:bg-blue-700"} transition-colors`}
            >
              <Plus className="h-4 w-4 mr-2" />
          ...
          <div className="overflow-x-auto">
            <table
              className={`min-w-full divide-y ${isLuxury ? "divide-[#D4AF37]/10" : "divide-gray-200"}`}
            >
              <thead className={isLuxury ? "bg-[#f9f9f9]" : "bg-gray-50"}>
                <tr>
                  <th
                    className={`px-6 py-3 text-left text-xs font-medium ${isLuxury ? "text-[#b3932d]" : "text-gray-500"} uppercase tracking-wider`}
                  >
                    Nº Visita
                  </th>
                  <th
                  ...
                  <th
                    className={`px-6 py-3 text-right text-xs font-medium ${isLuxury ? "text-[#b3932d]" : "text-gray-500"} uppercase tracking-wider`}
                  >
                    Acciones
                  </th>
                </tr>
              ...
              <tbody
                className={`${isLuxury ? "bg-transparent divide-y divide-[#D4AF37]/10" : "bg-white divide-y divide-gray-200"}`}
              >
                {visits.map((visit) => (
                  <tr
                    key={visit.id}
                    className={
                      isLuxury ? "hover:bg-[#D4AF37]/5" : "hover:bg-gray-50"
                    }
                  >
                    <td
                      className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${isLuxury ? "text-[#1a1a1a]" : "text-gray-900"}`}
                    >
                      #{visit.visitNumber}
                    </td>
                    <td
                      className={`px-6 py-4 whitespace-nowrap text-sm ${isLuxury ? "text-gray-600" : "text-gray-900"}`}
                    >
                    ...
                    <td
                      className={`px-6 py-4 text-sm ${isLuxury ? "text-gray-600" : "text-gray-900"}`}
                    >
                    ...
                    <td
                      className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${isLuxury ? "text-[#D4AF37]" : "text-gray-900"}`}
                    >
                    ...
                    >
                      {visit.nextVisitFrom
                        ? new Date(visit.nextVisitFrom).toLocaleDateString(
                            "es-MX",
                          )
                        : "Sin fecha"}
                    </td>
