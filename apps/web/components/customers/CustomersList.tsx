...
import CustomerFileHeader, {
  type CustomerFileHeaderHandle,
} from "@/components/customers/CustomerFileHeader";
...
export default function CustomersList({
  ...
  useEffect(() => {
    async function fetchCustomers() {
      try {
        ...
        console.warn("[CustomersList] Fetching from:", url);
  ...
  const getNextAppointmentDisplay = (customer: Customer) => {
    ...
    if (customer.lastVisit) {
      ...
      return (
        <div
          className="text-sm text-orange-600 bg-orange-50 px-2 py-1 rounded inline-block"
          title="Estimada: Última Visita + 15 días"
        >
  ...
  const filteredCustomers = inlineSearch
    ? customers.filter(
        (c) =>
          ...
          (c.email &&
            c.email.toLowerCase().includes(inlineSearch.toLowerCase())),
      )
    : customers;
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            {searchParams.sort && columnLabels[searchParams.sort] && (
              ...
              >
                <span>
                  Ordenado por:{" "}
                  <strong>{columnLabels[searchParams.sort]}</strong>
                  {searchParams.order === "asc" ? " (A-Z)" : " (Z-A)"}
                </span>
                <span className="text-gray-400 font-bold hover:text-gray-600">
                  ×
                </span>
              </button>
            )}
                ...
                <tr key={customer.id} className="hover:bg-gray-50">
                      ...
                      <div className="ml-4 overflow-hidden flex flex-col items-start gap-1">
                        ...
                        {customer.medicalHistory &&
                          ((customer.medicalHistory.allergies?.length || 0) >
                            0 ||
                            (customer.medicalHistory.conditions?.length || 0) >
                              0) && (
                            <div
                              title="Precauciones Médicas"
                              className="flex items-center gap-1 bg-red-50 text-red-600 border border-red-200 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide"
                            >
                  ...
                  <td className="hidden xl:table-cell px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-[#C5A059] bg-amber-50 px-2 py-1 rounded border border-[#C5A059]/30 inline-block">
                      $
                      {((customer as any).priceToCharge || 0).toLocaleString(
                        "es-MX",
