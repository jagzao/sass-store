-- =================================================================
-- MIGRACIÓN DIRECTA PARA SUPABASE - SISTEMA DE INVENTARIO
-- =================================================================
-- Este script crea las tablas necesarias para el sistema de inventario
-- Es seguro para producción porque solo crea tablas si no existen
-- =================================================================
...
CREATE TRIGGER IF NOT EXISTS update_product_inventory_updated_at
    BEFORE UPDATE ON product_inventory
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Trigger para service_products
CREATE TRIGGER IF NOT EXISTS update_service_products_updated_at
    BEFORE UPDATE ON service_products
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Trigger para inventory_alerts
CREATE TRIGGER IF NOT EXISTS update_inventory_alerts_updated_at
    BEFORE UPDATE ON inventory_alerts
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Trigger para product_alert_config
CREATE TRIGGER IF NOT EXISTS update_product_alert_config_updated_at
...
CREATE POLICY IF NOT EXISTS tenant_isolation_product_inventory ON product_inventory
    FOR ALL TO app_user
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());

-- Políticas RLS para service_products
CREATE POLICY IF NOT EXISTS tenant_isolation_service_products ON service_products
    FOR ALL TO app_user
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());

-- Políticas RLS para inventory_transactions
CREATE POLICY IF NOT EXISTS tenant_isolation_inventory_transactions ON inventory_transactions
    FOR ALL TO app_user
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());

-- Políticas RLS para inventory_alerts
CREATE POLICY IF NOT EXISTS tenant_isolation_inventory_alerts ON inventory_alerts
    FOR ALL TO app_user
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());

-- Políticas RLS para product_alert_config
CREATE POLICY IF NOT EXISTS tenant_isolation_product_alert_config ON product_alert_config
    FOR ALL TO app_user
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());
