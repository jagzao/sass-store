...
async function testRLS() {
  ...
  try {
    ...
    const tenantsResult = await client.query(
      `SELECT id, slug, name FROM tenants LIMIT 2`,
    );
    ...
    console.log(
      `DEBUG: current_tenant_id is set to: ${debugQuery.rows[0].current_tenant}`,
    );

    const tenant1Products = await client.query(
    ...
    if (tenant1Products.rows.some((p) => p.tenant_id !== tenant1.id)) {
      console.error(
        "❌ RLS FAIL: Tenant 1 query returned products from another tenant.",
      );
      return;
    }
    ...
    if (tenant2Products.rows.some((p) => p.tenant_id !== tenant2.id)) {
      console.error(
        "❌ RLS FAIL: Tenant 2 query returned products from another tenant.",
      );
      return;
    }
    ...
    if (overlap.length > 0) {
      console.error(
        `❌ RLS FAIL: ${overlap.length} products leaked between tenants.`,
      );
      return;
    }
    ...
    if (noContextProducts.rows.length > 0) {
      console.warn(
        `⚠️  RLS Warning: Found ${noContextProducts.rows.length} products without a tenant context. This may be intended.`,
      );
    } else {
