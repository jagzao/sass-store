...
async function debugRLS() {
  ...
  const userInfo: any = await db.execute(
    sql`SELECT current_user, current_database(), session_user`,
  );
  ...
  await db.execute(
    sql`SELECT set_config('app.current_tenant_id', '833e18a5-4f97-4291-93f3-3287cceccd5c', FALSE)`,
  );

  const contextCheck: any = await db.execute(
    sql`SELECT current_setting('app.current_tenant_id', true) as tenant_id`,
  );
  console.log("Context after setting:", contextCheck[0]);

  // Try a simple query
  const testQuery: any = await db.execute(
  ...
  // Test the function
  try {
    const funcTest: any = await db.execute(
      sql`SELECT get_current_tenant_id() as result`,
    );
