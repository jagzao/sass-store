...
async function deleteServiceTestSalonTenants() {
  console.log("Searching for 'Service Test Salon' tenants...");

  // Use query builder to find tenants with name "Service Test Salon"
  const tenantsToDelete = await db
    .select({ id: tenants.id, name: tenants.name, slug: tenants.slug })
    .from(tenants)
    .where(ilike(tenants.name, "%Service Test Salon%"));

  if (tenantsToDelete.length === 0) {
    console.log("No tenants found with name containing 'Service Test Salon'");
    process.exit(0);
  }
...
deleteServiceTestSalonTenants().catch((err) => {
