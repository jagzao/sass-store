...
async function testRLSIsolation() {
  ...
  try {
    ...
    if (invalidProducts.length > 0) {
      ...
      console.error(
        "   Invalid products:",
        invalidProducts.map((p) => ({ id: p.id, tenantId: p.tenantId })),
      );
      process.exit(1);
    }
    ...
    const invalidTenant2Products = tenant2Products.filter(
    ...
    if (invalidTenant2Products.length > 0) {
      ...
      console.error(
        "   Invalid products:",
        invalidTenant2Products.map((p) => ({ id: p.id, tenantId: p.tenantId })),
      );
      process.exit(1);
    }
    ...
    const tenant1Services = await withTenantContext(
      db,
      tenant1.id,
      async (db) => {
    ...
    console.log(
      `   Found ${tenant1Services.length} services for ${tenant1.name}`,
    );

    const invalidServices = tenant1Services.filter(
      (s) => s.tenantId !== tenant1.id,
    );
    if (invalidServices.length > 0) {
      console.error("   ❌ FAIL: Found services from other tenants!");
      console.error(
        "   Invalid services:",
        invalidServices.map((s) => ({ id: s.id, tenantId: s.tenantId })),
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.log(
        `   ⚠️  WARNING: Query without tenant context returned ${allProducts.length} products`,
      );
      console.log(
        "   This is OK if RLS is enforcing empty results when context is not set",
      );
      console.log("   But it's better to always use withTenantContext()\n");
    } catch (error) {
      console.log(
        "   ℹ️  INFO: Query without tenant context failed (expected behavior)",
      );
      console.log(
