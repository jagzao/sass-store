...
const fixes: Fix[] = [
  {
    ...
    description:
      "Replace Math.random() with crypto.randomUUID() for IDs and tokens",
    pattern:
      /const\s+(\w+)\s*=\s*['"`](\w+)_\$\{Date\.now\(\)\}_\$\{Math\.random\(\)\.toString\(36\)\.slice\(2\)\}['"`]/g,
    replacement:
      "const $1 = `$2_${Date.now()}_${crypto.randomUUID().replace(/-/g, '')}`",
  ...
  },
  {
    ...
    description:
      "Remove or redact console.log statements with passwords, tokens, keys",
    pattern:
  ...
  },
  {
    ...
    replacement:
      'cookies().set($1, $2, { httpOnly: true, sameSite: "strict", secure: true })',
    filePattern: "**/*.{ts,tsx,js,jsx}",
    severity: "high",
  },
];
...
async function runAutoFix(): Promise<void> {
      ...
      for (const file of files) {
        try {
          ...
          if (content !== newContent && changeCount > 0) {
            ...
            console.log(
              `   ✅ Fixed: ${path.relative(process.cwd(), file)} (${changeCount} change(s))`,
            );
  ...
  if (results.length === 0) {
    ...
    console.log(
      `✅ Successfully fixed ${totalChanges} issue(s) across ${uniqueFiles} file(s)\n`,
    );
  ...
  console.log(
    '4. Commit changes: git add . && git commit -m "security: auto-fix security issues"',
  );
  console.log("\n✨ Auto-fix completed!\n");
}
