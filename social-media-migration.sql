...
-- Tabla de campañas sociales (extendiendo la tabla campaigns existente)
-- Nota: Como la tabla campaigns ya existe, solo añadimos una columna para diferenciar tipo
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'campaigns' AND column_name = 'campaign_type') THEN
        RAISE NOTICE 'La columna campaign_type ya existe en la tabla campaigns';
    ELSE
        ALTER TABLE campaigns ADD COLUMN campaign_type VARCHAR(20) DEFAULT 'general' CHECK (campaign_type IN ('general', 'social', 'seasonal', 'promotion'));
        RAISE NOTICE 'Columna campaign_type agregada a la tabla campaigns';
    END IF;
END $$;
